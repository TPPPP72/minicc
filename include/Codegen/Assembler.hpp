...
public:
    ...
    static void loc(std::uint32_t line) { stream() << std::format("  .loc 1 {}\n", line); }
    static void string_lit(std::string_view escaped) { stream() << std::format("  .string \"{}\"\n", escaped); }

    template <typename T>
    static void push(T op) { stream() << std::format("  push {}\n", format_op(op)); }
    template <typename T>
    static void pop(T op) { stream() << std::format("  pop {}\n", format_op(op)); }
    template <typename T>
    static void neg(T op) { stream() << std::format("  neg {}\n", format_op(op)); }
    template <typename T>
    static void sete(T op) { stream() << std::format("  sete {}\n", format_op(op)); }
    template <typename T>
    static void setne(T op) { stream() << std::format("  setne {}\n", format_op(op)); }
    template <typename T>
    static void setl(T op) { stream() << std::format("  setl {}\n", format_op(op)); }
    template <typename T>
    static void setle(T op) { stream() << std::format("  setle {}\n", format_op(op)); }
    template <typename T>
    static void setg(T op) { stream() << std::format("  setg {}\n", format_op(op)); }
    template <typename T>
    static void setge(T op) { stream() << std::format("  setge {}\n", format_op(op)); }
